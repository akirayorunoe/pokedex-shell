...
function useHref(to, { relative } = {}) {
  ...
  let { basename, navigator: navigator2 } = React2$1.useContext(NavigationContext);
...
function useNavigationType() {
  return React2$1.useContext(LocationContext).navigationType;
}
...
function useOutletContext() {
  return React2$1.useContext(OutletContext);
}
...
function useRoutesImpl(routes, locationArg, dataRouterState, future) {
  ...
  let { navigator: navigator2, static: isStatic } = React2$1.useContext(NavigationContext);
...
function useAsyncValue() {
  let value = React2$1.useContext(AwaitContext);
  return value?._data;
}
...
function useBlocker(shouldBlock) {
  ...
  let [blockerKey, setBlockerKey] = React2$1.useState("");
  let blockerFunction = React2$1.useCallback(
    (arg) => {
